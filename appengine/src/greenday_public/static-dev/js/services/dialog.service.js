...
(function() {
  ...
  function DialogService($q, $mdDialog) {
    ...
    /* istanbul ignore next */
    /** @ngInject */
    function addCollectionController(
      $scope,
      $timeout,
      $analytics,
      PageService,
      ...
      ctrl.addCollection = function() {
        ...
        CollectionModel.create(ctrl.collection).then(function() {
    ...
    /* istanbul ignore next */
    /** @ngInject */
    function addCollaboratorsController(
      $scope,
      PageService,
      CollaboratorService,
      UserService,
      ...
      CollaboratorService.bindAll(projectFilter, $scope, "ctrl.collaborators");

      // get current project collaborators.
      ctrl.getProjectCollaborators = function() {
        ctrl.isBusy = true;
        CollaboratorService.list(projectId).then(function() {
          ctrl.isBusy = false;
        });
      };
      ...
      $scope.$watch(
        ...
        function(newVal, oldVal) {
          if (newVal && newVal.email && newVal.email !== oldVal.email) {
            ctrl.isBusy = true;
            UserService.queryUserContacts(newVal.email)
              .then(function(users) {
                // filter out users already assigned to the project
                var assignedUserEmails = _.pluck(ctrl.collaborators, "email"),
                  autocompleteEmails = _.transform(users, function(
                    result,
                    user
                  ) {
                    if (
                      angular.isDefined(user.gd$email) &&
                      user.gd$email.length &&
                      user.gd$email[0].address
                    ) {
                      result.push({
                        name: user.title.$t,
                ...
                ctrl.possibleUsers = _.difference(
                  autocompleteEmails,
                  assignedUserEmails
                );
              })
      ...
      ctrl.addCollaborator = function() {
        ...
        CollaboratorService.create(ctrl.newCollaborator).then(function() {
          var metadata = {
      ...
      ctrl.removeCollaborator = function(collaborator) {
        ctrl.isBusy = true;
        CollaboratorService.remove(collaborator.id).then(function() {
          ctrl.isBusy = false;
        });
      };
    ...
    function keepSettingsController($scope, PageService) {
      var ctrl = this,
        projectId = PageService.getPageData().projectId,
        projectFilter = { project_id: projectId };
